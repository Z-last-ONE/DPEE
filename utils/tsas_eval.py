...
def predict_events(model, device, idx, content: str, token, seg, mask, seq_len, id_type: dict,
    ...
    for type_pred_one in type_pred:
        ...
        # plt.imshow(t2a_logits[0], cmap=plt.cm.winter, vmin=t2a_logits.min(), vmax=t2a_logits.max())
        # plt.colorbar()
        # plt.show()
        # plt.imshow(role_logits[0], cmap=plt.cm.winter, vmin=role_logits.min(), vmax=role_logits.max())
        # plt.colorbar()
        # plt.show()
    ...
    for tri_s, tri_e, e_t in event_3tuple:
        ...
        if flag_need_spilt:
            '''至此，一个event已经被解析出来，需要看是否进行拆分'''
            '''析出全部的论元，然后通过role矩阵进行关系解码'''
            example_graph = []
            argument_list = []
            ...
            for i, argument_tuple_source in enumerate(argument_list):
                for j, argument_tuple_target in enumerate(argument_list):
                    if i != j:
                        ...
                        is_relation = True if area_matrix_value_mean > -9999999 else False
                        if is_relation:
...
def calculate_confused_score(gold_event_list, predict_event_list):
    ...
    for i in range(len(col_ind)):
        if sample_cost[row_ind[i], col_ind[i]] < 99998:
            '''至此确定了gold_list与predict_list的事件对应关系，接下来计算F1值'''
